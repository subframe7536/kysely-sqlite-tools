...
//#endregion
//#region ../node_modules/.pnpm/@sqlite.org+sqlite-wasm@3.53.0-build1/node_modules/@sqlite.org/sqlite-wasm/dist/sqlite3-worker1.mjs
/* @preserve
**
...
async function sqlite3InitModule(moduleArg = {}) {
	...
	*/
	...
	function findWasmBinary() {
		...
		return new URL("" + new URL("sqlite3-BVKGSWc-.wasm", import.meta.url).href, "" + import.meta.url).href;
	}
	...
	var MEMFS = {
		...
		createNode(parent, name, mode, dev) {
			...
			if (FS.isDir(node.mode)) {
				...
				node.usedBytes = 0;
				node.contents = MEMFS.emptyFileContents ?? (MEMFS.emptyFileContents = /* @__PURE__ */ new Uint8Array(0));
		...
		*/
		...
		globalThis.WhWasmUtilInstaller = function WhWasmUtilInstaller(target) {
			...
			*/
			...
			const __ptrIR = target.pointerIR ?? (target.pointerIR = "i32");
			const __ptrSize = target.pointerSize ?? (target.pointerSize = "i32" === __ptrIR ? 4 : "i64" === __ptrIR ? 8 : 0);
		...
		globalThis.Jaccwabyt = function StructBinderFactory(config) {
			...
			* descriptor. */
			...
			const looksLikeASig = function f(sig) {
				f.rxSig1 ?? (f.rxSig1 = /^[ipPsjfdcC]$/);
				f.rxSig2 ?? (f.rxSig2 = /^[vipPsjfdcC]\([ipPsjfdcC]*\)$/);
			...
			*/
			const makeMemberStructWrapper = function callee(ctor, name, si) {
				...
				*/
				const __innerStructs = callee.innerStructs ?? (callee.innerStructs = /* @__PURE__ */ new Map());
				...
				si.structName ?? (si.structName = ctor.structName + "::" + name);
				si.key = key;
				si.name = name;
			...
			const StructBinderImpl = function StructBinderImpl(structName, si, opt = Object.create(null)) {
				...
				*/
				...
				if (1 === arguments.length) {
					...
					var _si;
					si = ads(si);
					(_si = si).name ?? (_si.name = structName);
				} else si = ads(si);
				structName ?? (structName = si.structName);
				structName ?? (structName = opt.structName);
				...
				if (!autoCalc) {
					...
					(_si2 = si).offset ?? (_si2.offset = 0);
				} else (_si3 = si).offset ?? (_si3.offset = 0);
		...
		globalThis.sqlite3ApiBootstrap.initializers.push(function(sqlite3) {
			...
			*/
			cache.memBuffer = (id = 0) => {
				var _cache$buffer$pool;
				return (_cache$buffer$pool = cache.buffer.pool)[id] ?? (_cache$buffer$pool[id] = wasm.alloc(cache.buffer.n));
			};
			...
			var _KVVfsStorage_brand = /* @__PURE__ */ new WeakSet();
			/**
			...
			*/
			class KVVfsStorage {
				constructor() {
					_classPrivateMethodInitSpec(this, _KVVfsStorage_brand);
					_classPrivateFieldInitSpec(this, _map, Object.create(null));
					_classPrivateFieldInitSpec(this, _keys, null);
					_classPrivateFieldInitSpec(this, _size, 0);
					this.clear();
				}
				key(n) {
					if (n < 0 || n >= _classPrivateFieldGet2(_size, this)) return null;
					return _assertClassBrand(_KVVfsStorage_brand, this, _getKeys).call(this)[n];
				}
				getItem(k) {
					return _classPrivateFieldGet2(_map, this)[k] ?? null;
				}
				setItem(k, v) {
					if (!(k in _classPrivateFieldGet2(_map, this))) {
						var _this$size;
						_classPrivateFieldSet2(_size, this, (_this$size = _classPrivateFieldGet2(_size, this), ++_this$size));
						_classPrivateFieldSet2(_keys, this, null);
					}
					_classPrivateFieldGet2(_map, this)[k] = "" + v;
				}
				removeItem(k) {
					if (k in _classPrivateFieldGet2(_map, this)) {
						var _this$size2;
						delete _classPrivateFieldGet2(_map, this)[k];
						_classPrivateFieldSet2(_size, this, (_this$size2 = _classPrivateFieldGet2(_size, this), --_this$size2));
						_classPrivateFieldSet2(_keys, this, null);
					}
				}
				clear() {
					_classPrivateFieldSet2(_map, this, Object.create(null));
					_classPrivateFieldSet2(_keys, this, null);
			...
			function _getKeys() {
				return _classPrivateFieldGet2(_keys, this) ?? _classPrivateFieldSet2(_keys, this, Object.keys(_classPrivateFieldGet2(_map, this)));
			}
					...
					for (const ear of store.listeners) {
						...
						if (f) {
							...
							if ("write" === eventName && ear.decodePages && +args[0] > 0) {
								...
								u8enc ?? (u8enc = new TextEncoder("utf-8"));
								z0 ?? (z0 = cache.memBuffer(10));
								z1 ?? (z1 = cache.memBuffer(11));
								...
								if (rc > 0) {
									wcache ?? (wcache = Object.create(null));
 ...
 /** The xAccess method returns [SQLITE_OK] on success or some
			...
			*/
			const sqlite3_js_kvvfs_listen = function(opt) {
				...
				if (opt.events) ((_store = store).listeners ?? (_store.listeners = [])).push(opt);
			};
			/**
			...
			if (sqlite3.__isUnderTest && sqlite3.vtab) {
				...
				*/
				...
				const theModule = function f() {
					return f.mod ?? (f.mod = new sqlite3.capi.sqlite3_module().setupModule({
		...
		globalThis.sqlite3ApiBootstrap.initializers.push(function(sqlite3) {
			...
			*/
			opfsUtil.getRootDir = async function f() {
				return f.promise ?? (f.promise = navigator.storage.getDirectory().then((d) => {
			...
			*/
			opfsUtil.initOptions = function callee(vfsName, options) {
				...
				(_options = options).verbose ?? (_options.verbose = urlParams.has("opfs-verbose") ? +urlParams.get("opfs-verbose") : 1);
				(_options2 = options).sanityChecks ?? (_options2.sanityChecks = urlParams.has("opfs-sanity-check"));
				...
				(_options3 = options).proxyUri ?? (_options3.proxyUri = opfsUtil.proxyUri);
 ...
 /**
		...
		globalThis.sqlite3ApiBootstrap.initializers.push(function(sqlite3) {
			...
			*/
			...
			var _dhVfsRoot = /* @__PURE__ */ new WeakMap();
			var _dhOpaque = /* @__PURE__ */ new WeakMap();
			var _dhVfsParent = /* @__PURE__ */ new WeakMap();
			var _mapSAHToName = /* @__PURE__ */ new WeakMap();
			var _mapFilenameToSAH = /* @__PURE__ */ new WeakMap();
			var _availableSAH = /* @__PURE__ */ new WeakMap();
			var _mapS3FileToOFile_ = /* @__PURE__ */ new WeakMap();
			var _apBody = /* @__PURE__ */ new WeakMap();
			var _dvBody = /* @__PURE__ */ new WeakMap();
			var _cVfs = /* @__PURE__ */ new WeakMap();
			var _verbosity = /* @__PURE__ */ new WeakMap();
			var _OpfsSAHPool_brand = /* @__PURE__ */ new WeakSet();
			/**
			...
			*/
			class OpfsSAHPool {
				constructor(options = Object.create(null)) {
					_classPrivateMethodInitSpec(this, _OpfsSAHPool_brand);
					_defineProperty(this, "vfsDir", void 0);
					_classPrivateFieldInitSpec(this, _dhVfsRoot, void 0);
					_classPrivateFieldInitSpec(this, _dhOpaque, void 0);
					_classPrivateFieldInitSpec(this, _dhVfsParent, void 0);
					_classPrivateFieldInitSpec(this, _mapSAHToName, /* @__PURE__ */ new Map());
					_classPrivateFieldInitSpec(this, _mapFilenameToSAH, /* @__PURE__ */ new Map());
					_classPrivateFieldInitSpec(this, _availableSAH, /* @__PURE__ */ new Set());
					_classPrivateFieldInitSpec(this, _mapS3FileToOFile_, /* @__PURE__ */ new Map());
					_classPrivateFieldInitSpec(this, _apBody, new Uint8Array(HEADER_CORPUS_SIZE));
					_classPrivateFieldInitSpec(this, _dvBody, void 0);
					_classPrivateFieldInitSpec(this, _cVfs, void 0);
					_classPrivateFieldInitSpec(this, _verbosity, void 0);
					_classPrivateFieldSet2(_verbosity, this, options.verbosity ?? optionDefaults.verbosity);
					this.vfsName = options.name || optionDefaults.name;
					_classPrivateFieldSet2(_cVfs, this, createOpfsVfs(this.vfsName));
					setPoolForVfs(_classPrivateFieldGet2(_cVfs, this).pointer, this);
					this.vfsDir = options.directory || "." + this.vfsName;
					_classPrivateFieldSet2(_dvBody, this, new DataView(_classPrivateFieldGet2(_apBody, this).buffer, _classPrivateFieldGet2(_apBody, this).byteOffset));
				...
				log(...args) {
					_assertClassBrand(_OpfsSAHPool_brand, this, _logImpl).call(this, 2, ...args);
				}
				warn(...args) {
					_assertClassBrand(_OpfsSAHPool_brand, this, _logImpl).call(this, 1, ...args);
				}
				error(...args) {
					_assertClassBrand(_OpfsSAHPool_brand, this, _logImpl).call(this, 0, ...args);
				}
				getVfs() {
					return _classPrivateFieldGet2(_cVfs, this);
				}
				getCapacity() {
					return _classPrivateFieldGet2(_mapSAHToName, this).size;
				}
				getFileCount() {
					return _classPrivateFieldGet2(_mapFilenameToSAH, this).size;
				}
				getFileNames() {
					const rc = [];
					for (const n of _classPrivateFieldGet2(_mapFilenameToSAH, this).keys()) rc.push(n);
					return rc;
				}
				/**
				...
				*/
				async addCapacity(n) {
					for (let i = 0; i < n; ++i) {
						...
						const ah = await (await _classPrivateFieldGet2(_dhOpaque, this).getFileHandle(name, { create: true })).createSyncAccessHandle();
						_classPrivateFieldGet2(_mapSAHToName, this).set(ah, name);
				...
				*/
				async reduceCapacity(n) {
					let nRm = 0;
					for (const ah of Array.from(_classPrivateFieldGet2(_availableSAH, this))) {
						if (nRm === n || this.getFileCount() === this.getCapacity()) break;
						const name = _classPrivateFieldGet2(_mapSAHToName, this).get(ah);
						ah.close();
						await _classPrivateFieldGet2(_dhOpaque, this).removeEntry(name);
						_classPrivateFieldGet2(_mapSAHToName, this).delete(ah);
						_classPrivateFieldGet2(_availableSAH, this).delete(ah);
						++nRm;
					}
					return nRm;
				}
				...
				*/
				releaseAccessHandles() {
					for (const ah of _classPrivateFieldGet2(_mapSAHToName, this).keys()) ah.close();
					_classPrivateFieldGet2(_mapSAHToName, this).clear();
					_classPrivateFieldGet2(_mapFilenameToSAH, this).clear();
					_classPrivateFieldGet2(_availableSAH, this).clear();
				}
				/**
				...
				*/
				async acquireAccessHandles(clearFiles = false) {
					const files = [];
					for await (const [name, h] of _classPrivateFieldGet2(_dhOpaque, this)) if ("file" === h.kind) files.push([name, h]);
					return Promise.all(files.map(async ([name, h]) => {
						try {
							...
							if (clearFiles) {
								...
								if (path) _classPrivateFieldGet2(_mapFilenameToSAH, this).set(path, ah);
								else _classPrivateFieldGet2(_availableSAH, this).add(ah);
							}
						} catch (e) {
							this.storeErr(e);
				...
				*/
				getAssociatedPath(sah) {
					sah.read(_classPrivateFieldGet2(_apBody, this), { at: 0 });
					const flags = _classPrivateFieldGet2(_dvBody, this).getUint32(HEADER_OFFSET_FLAGS);
					if (_classPrivateFieldGet2(_apBody, this)[0] && (flags & capi.SQLITE_OPEN_DELETEONCLOSE || (flags & PERSISTENT_FILE_TYPES) === 0)) {
						warn(`Removing file with unexpected flags ${flags.toString(16)}`, _classPrivateFieldGet2(_apBody, this));
						this.setAssociatedPath(sah, "", 0);
						return "";
					}
					const fileDigest = new Uint32Array(HEADER_DIGEST_SIZE / 4);
					sah.read(fileDigest, { at: HEADER_OFFSET_DIGEST });
					const compDigest = this.computeDigest(_classPrivateFieldGet2(_apBody, this), flags);
					if (fileDigest.every((v, i) => v === compDigest[i])) {
						const pathBytes = _classPrivateFieldGet2(_apBody, this).findIndex((v) => 0 === v);
						if (0 === pathBytes) sah.truncate(HEADER_OFFSET_DATA);
						return pathBytes ? textDecoder.decode(_classPrivateFieldGet2(_apBody, this).subarray(0, pathBytes)) : "";
					} else {
				...
				*/
				setAssociatedPath(sah, path, flags) {
					const enc = textEncoder.encodeInto(path, _classPrivateFieldGet2(_apBody, this));
					if (HEADER_MAX_PATH_SIZE <= enc.written + 1) toss("Path too long:", path);
					if (path && flags) flags |= FLAG_COMPUTE_DIGEST_V2;
					_classPrivateFieldGet2(_apBody, this).fill(0, enc.written, HEADER_MAX_PATH_SIZE);
					_classPrivateFieldGet2(_dvBody, this).setUint32(HEADER_OFFSET_FLAGS, flags);
					const digest = this.computeDigest(_classPrivateFieldGet2(_apBody, this), flags);
					sah.write(_classPrivateFieldGet2(_apBody, this), { at: 0 });
					sah.write(digest, { at: HEADER_OFFSET_DIGEST });
					sah.flush();
					if (path) {
						_classPrivateFieldGet2(_mapFilenameToSAH, this).set(path, sah);
						_classPrivateFieldGet2(_availableSAH, this).delete(sah);
					} else {
						sah.truncate(HEADER_OFFSET_DATA);
						_classPrivateFieldGet2(_availableSAH, this).add(sah);
					}
				}
				/**
				...
				*/
				async reset(clearFiles) {
					...
					_classPrivateFieldSet2(_dhVfsRoot, this, h);
					_classPrivateFieldSet2(_dhVfsParent, this, prev);
					_classPrivateFieldSet2(_dhOpaque, this, await _classPrivateFieldGet2(_dhVfsRoot, this).getDirectoryHandle(OPAQUE_DIR_NAME, { create: true }));
					this.releaseAccessHandles();
				...
				*/
				deletePath(path) {
					const sah = _classPrivateFieldGet2(_mapFilenameToSAH, this).get(path);
					if (sah) {
						_classPrivateFieldGet2(_mapFilenameToSAH, this).delete(path);
						this.setAssociatedPath(sah, "", 0);
					}
					return !!sah;
				}
				...
				*/
				nextAvailableSAH() {
					const [rc] = _classPrivateFieldGet2(_availableSAH, this).keys();
					return rc;
				}
				/**
				...
				*/
				getOFileForS3File(pFile) {
					return _classPrivateFieldGet2(_mapS3FileToOFile_, this).get(pFile);
				}
				/**
				...
				*/
				mapS3FileToOFile(pFile, file) {
					if (file) {
						_classPrivateFieldGet2(_mapS3FileToOFile_, this).set(pFile, file);
						setPoolForPFile(pFile, this);
					} else {
						_classPrivateFieldGet2(_mapS3FileToOFile_, this).delete(pFile);
				...
				*/
				hasFilename(name) {
					return _classPrivateFieldGet2(_mapFilenameToSAH, this).has(name);
				}
				/**
				...
				*/
				getSAHForPath(path) {
					return _classPrivateFieldGet2(_mapFilenameToSAH, this).get(path);
				}
				/**
				...
				*/
				async removeVfs() {
					if (!_classPrivateFieldGet2(_cVfs, this).pointer || !_classPrivateFieldGet2(_dhOpaque, this)) return false;
					capi.sqlite3_vfs_unregister(_classPrivateFieldGet2(_cVfs, this).pointer);
					_classPrivateFieldGet2(_cVfs, this).dispose();
					delete initPromises[this.vfsName];
					try {
						this.releaseAccessHandles();
						await _classPrivateFieldGet2(_dhVfsRoot, this).removeEntry(OPAQUE_DIR_NAME, { recursive: true });
						_classPrivateFieldSet2(_dhOpaque, this, void 0);
						await _classPrivateFieldGet2(_dhVfsParent, this).removeEntry(_classPrivateFieldGet2(_dhVfsRoot, this).name, { recursive: true });
						_classPrivateFieldSet2(_dhVfsRoot, this, _classPrivateFieldSet2(_dhVfsParent, this, void 0));
					} catch (e) {
				...
				*/
				pauseVfs() {
					if (_classPrivateFieldGet2(_mapS3FileToOFile_, this).size > 0) sqlite3.SQLite3Error.toss(capi.SQLITE_MISUSE, "Cannot pause VFS", this.vfsName, "because it has opened files.");
					if (_classPrivateFieldGet2(_mapSAHToName, this).size > 0) {
				...
				*/
				isPaused() {
					return 0 === _classPrivateFieldGet2(_mapSAHToName, this).size;
				}
				/**
				...
				*/
				async unpauseVfs() {
					if (0 === _classPrivateFieldGet2(_mapSAHToName, this).size) return this.acquireAccessHandles(false).then(() => capi.sqlite3_vfs_register(_classPrivateFieldGet2(_cVfs, this), 0), this);
					return this;
				}
				//! Documented elsewhere in this file.
				exportFile(name) {
					const sah = _classPrivateFieldGet2(_mapFilenameToSAH, this).get(name) || toss("File not found:", name);
				...
				async importDbChunked(name, callback) {
					const sah = _classPrivateFieldGet2(_mapFilenameToSAH, this).get(name) || this.nextAvailableSAH() || toss("No available handles to import to.");
					sah.truncate(0);
			...
			function _logImpl(level, ...args) {
				if (_classPrivateFieldGet2(_verbosity, this) > level) loggers[level](this.vfsName + ":", ...args);
			}
			...
			*/
			class OpfsSAHPoolUtil {
				constructor(sahPool) {
					...
					_classPrivateFieldSet2(_p, this, sahPool);
					this.vfsName = sahPool.vfsName;
				}
				async addCapacity(n) {
					return _classPrivateFieldGet2(_p, this).addCapacity(n);
				}
				async reduceCapacity(n) {
					return _classPrivateFieldGet2(_p, this).reduceCapacity(n);
				}
				getCapacity() {
					return _classPrivateFieldGet2(_p, this).getCapacity(_classPrivateFieldGet2(_p, this));
				}
				getFileCount() {
					return _classPrivateFieldGet2(_p, this).getFileCount();
				}
				getFileNames() {
					return _classPrivateFieldGet2(_p, this).getFileNames();
				}
				async reserveMinimumCapacity(min) {
					const c = _classPrivateFieldGet2(_p, this).getCapacity();
					return c < min ? _classPrivateFieldGet2(_p, this).addCapacity(min - c) : c;
				}
				exportFile(name) {
					return _classPrivateFieldGet2(_p, this).exportFile(name);
				}
				importDb(name, bytes) {
					return _classPrivateFieldGet2(_p, this).importDb(name, bytes);
				}
				async wipeFiles() {
					return _classPrivateFieldGet2(_p, this).reset(true);
				}
				unlink(filename) {
					return _classPrivateFieldGet2(_p, this).deletePath(filename);
				}
				async removeVfs() {
					return _classPrivateFieldGet2(_p, this).removeVfs();
				}
				pauseVfs() {
					_classPrivateFieldGet2(_p, this).pauseVfs();
					return this;
				}
				async unpauseVfs() {
					return _classPrivateFieldGet2(_p, this).unpauseVfs().then(() => this);
				}
				isPaused() {
					return _classPrivateFieldGet2(_p, this).isPaused();
				}
			}
			/**
