...
//#endregion
//#region ../node_modules/.pnpm/@subframe7536+sqlite-wasm@1.3.1/node_modules/@subframe7536/sqlite-wasm/dist/index.js
...
//#endregion
//#region ../packages/dialect-generic-sqlite/dist/utils-P3bX5on0.mjs
...
function createGenericOnMessageCallback(init, post, custom) {
	let db;
	let closing = false;
	...
	const cancel = async (id, streamId) => {
	...
	const pull = async (id, streamId, chunkSize) => {
		const iterator = streams.get(streamId);
		if (!iterator) throw new Error(`Unknown stream: ${streamId}`);
		const rows = [];
	...
	return async (message) => {
		...
		await enqueue(async () => {
			try {
				if (closing && message.type !== "close") throw new Error("Worker is closing");
				switch (message.type) {
					case "init":
						...
						return;
					case "execute":
						if (!db) throw new Error("Worker is not initialized");
						send({
							type: "result",
							id: message.id,
					...
					case "stream":
						...
						if (!Number.isInteger(message.chunkSize) || message.chunkSize <= 0) throw new RangeError("chunkSize must be a positive integer");
						streams.set(message.streamId, db.iterator(message.isSelect, message.sql, message.parameters, message.chunkSize));
					...
					case "close":
						closing = true;
						await Promise.all([...streams.values()].map(async (iterator) => {
							try {
								await iterator.return?.();
							} catch {}
						}));
						...
						return;
					case "custom":
						if (!db || !custom) throw new Error(`Unknown worker request: ${message.name}`);
						send({
							type: "custom",
							id: message.id,
							result: await custom(db, {
								type: message.name,
...
function createWebOnMessageCallback(init, custom) {
	const callback = createGenericOnMessageCallback(init, (value) => globalThis.postMessage(value), custom);
	const onMessage = ({ data }) => void callback(data);
	if ("addEventListener" in globalThis) globalThis.addEventListener("message", onMessage);
	else globalThis.onmessage = onMessage;
}
//#endregion
...
//#endregion
//#region ../packages/dialect-wasqlite-worker/dist/utils-CNdcKWL1.js
/**
...
const defaultCreateDatabaseFn = async ({ fileName, url, useOPFS }) => {
	return initSQLiteCore((useOPFS ? (await import("./opfs-ATNUaWBR.js")).useOpfsStorage : (await import("./idb-B4PHe08a.js")).useIdbStorage)(fileName, { url }));
};
/**
...
* ```
*/
function createOnMessageCallback(create, custom) {
